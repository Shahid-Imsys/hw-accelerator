...
//  Date of Generation : Wed May 04 16:39:10 CEST 2022                 
//                                                                     
//---------------------------------------------------------------------
//   --------------------------------------------------------------     
...
SNPS_SP_HD_256x32_behav  top_behav (  .Q( Q_BEHAV), .ADR( ADR), .D( D), .WE( WE), .ME( ME), .CLK( CLK), .TEST1( TEST1), .TEST_RNM( TEST_RNM), .RME( RME), .RM( RM), .WA( WA), .WPULSE( WPULSE), .LS( LS), .BC0( BC0), .BC1( BC1), .BC2( BC2) );
SNPS_SP_HD_256x32  top_struct (  .Q( Q), .ADR( ADR), .D( D), .WE( WE), .ME( ME), .CLK( CLK), .TEST1( TEST1), .TEST_RNM( TEST_RNM), .RME( RME), .RM( RM), .WA( WA), .WPULSE( WPULSE), .LS( LS), .BC0( BC0), .BC1( BC1), .BC2( BC2) );
`define True    1'b1
`define False   1'b0
`define Mid     1'bx
`define Highz   1'bz
initial begin
  ...
  //$vcdpluson; 
    ...
    //$sdf_annotate( "SNPS_SP_HD_256x32.sdf");
end
task print_tdcs;
input count;
...
task write_mem;
input [7:0] addr_incr; //address increments by this value
input [7:0] address_w; //if null,entire memory is written.
...
task read_mem;
input [7:0] addr_incr; //address increments by this value
input [7:0] address_r; //if null,entire memory is read.
input [31:0] data; //if null,data=0 in case of single location,else data=addr.
integer k;
integer j;
integer l;
...
task read_mem_inv;
input [7:0] addr_incr; //address increments by this value
input [7:0] addr; //if null,entire memory is read.
input [31:0] data; //if null,data=0 in case of single location,else data=addr.
integer k;
integer j;
reg [31:0]  i;
 ...
 if (signal == "ME_X")
  ...
  begin
    ...
    begin
      if ( chk == 2'b11 )
      begin
        read_mem("",8'd255,32'bx);
        write_mem("",8'd255,32'b0);
      end
      else
      begin
        read_mem("",8'd0,32'bx);
        write_mem("",8'd0,32'b0);
      end
    end
...
begin
  ...
  begin
    if ( memstat == 2'b00 )
      read_mem("",8'd0,32'b00000000000000000000000000000000);
    else if ( memstat == 2'b01 )
      read_mem(1,"","");
    else if ( memstat == 2'b10 )
    begin
...
task coverage_1;
 ...
 @(posedge CLK) begin
     ADR[7:0] = pattern_reg[7:0];
     D[31:0] = pattern_reg[39:8];
     WE = pattern_reg[40];
     ME = pattern_reg[41];
     TEST1 = pattern_reg[42];
     TEST_RNM = pattern_reg[43];
     RME = pattern_reg[44];
     RM[3:0] = pattern_reg[48:45];
     WA[1:0] = pattern_reg[50:49];
     WPULSE[2:0] = pattern_reg[53:51];
     LS = pattern_reg[54];
     BC0 = pattern_reg[55];
     BC1 = pattern_reg[56];
     BC2 = pattern_reg[57];
...
task coverage_2;
  ...
  @(negedge CLK) begin
     #5;
     ADR[7:0] = add_data_pattern[7:0];
     D[31:0] = add_data_pattern[39:8];
     WE = control_sig[0];
     ME = control_sig[1];
     TEST1 = control_sig[2];
     LS <= control_sig[3];
...
begin
 ...
 $display("@%t  ============= When BC0 = %d ================ ", $time, bc0_i);
  ...
  write_mem("",8'b0,32'd0);
  @(negedge CLK);
  ADR = 1;
  $display("@%t    Writing 32'b11111111111111111111111111111111 at %b", $time,ADR);
  write_mem("",8'b1,32'b11111111111111111111111111111111);
...
WPULSE = 3'b000;
  ...
  read_mem("",8'b1, 32'b11111111111111111111111111111111);
...
WPULSE = 3'b000;
 ...
 $display("Non recommended warning messages may come in simulation log file.");
  ...
  $display("@%t    Writing in Memory data = 32'b10101010101010101010101010101010 during TEST1 active", $time);
   write_mem(1,8'bx,32'b10101010101010101010101010101010); 
  @(negedge CLK);
  @(negedge CLK);
   read_mem_test1(1,8'bx,32'b10101010101010101010101010101010); 
  if (err_cnt == 0)
  begin
...
WPULSE = 3'b000;
  ...
  $display ("\n#######################################################################");
  test_num = 158;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Constraint on D[3 : 0](Td1c_h) in Write mode  --- \n\n", $time, test_num);
  ...
  $display ("\n#######################################################################");
  test_num = 159;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Constraint on D[7 : 4](Td2c_h) in Write mode  --- \n\n", $time, test_num);
  ...
  $display ("\n#######################################################################");
  test_num = 160;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Constraint on D[11 : 8](Td3c_h) in Write mode  --- \n\n", $time, test_num);
  ...
  $display ("\n#######################################################################");
  test_num = 161;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Constraint on D[15 : 12](Td4c_h) in Write mode  --- \n\n", $time, test_num);
  ...
  $display ("\n#######################################################################");
  test_num = 162;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Constraint on D[19 : 16](Td5c_h) in Write mode  --- \n\n", $time, test_num);
  ...
  $display ("\n#######################################################################");
  test_num = 163;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Constraint on D[23 : 20](Td6c_h) in Write mode  --- \n\n", $time, test_num);
  ...
  $display ("\n#######################################################################");
  test_num = 164;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Constraint on D[27 : 24](Td7c_h) in Write mode  --- \n\n", $time, test_num);
  ...
  $display ("\n#######################################################################");
  test_num = 165;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Constraint on D[31 : 28](Td8c_h) in Write mode  --- \n\n", $time, test_num);
  ...
  $display ("\n#######################################################################");
  test_num = 166;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[3 : 0](Td1c_h) in Write mode  --- \n\n", $time, test_num);
     write_mem("",8'd0, 32'd0);
  for ( d_num = 0; d_num <= 3; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 167;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[7 : 4](Td2c_h) in Write mode  --- \n\n", $time, test_num);
     write_mem("",8'd0, 32'd0);
  for ( d_num = 4; d_num <= 7; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 168;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[11 : 8](Td3c_h) in Write mode  --- \n\n", $time, test_num);
     write_mem("",8'd0, 32'd0);
  for ( d_num = 8; d_num <= 11; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 169;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[15 : 12](Td4c_h) in Write mode  --- \n\n", $time, test_num);
     write_mem("",8'd0, 32'd0);
  for ( d_num = 12; d_num <= 15; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 170;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[19 : 16](Td5c_h) in Write mode  --- \n\n", $time, test_num);
     write_mem("",8'd0, 32'd0);
  for ( d_num = 16; d_num <= 19; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 171;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[23 : 20](Td6c_h) in Write mode  --- \n\n", $time, test_num);
     write_mem("",8'd0, 32'd0);
  for ( d_num = 20; d_num <= 23; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 172;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[27 : 24](Td7c_h) in Write mode  --- \n\n", $time, test_num);
     write_mem("",8'd0, 32'd0);
  for ( d_num = 24; d_num <= 27; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 173;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[31 : 28](Td8c_h) in Write mode  --- \n\n", $time, test_num);
     write_mem("",8'd0, 32'd0);
  for ( d_num = 28; d_num <= 31; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 174;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Constraint on D[3 : 0](Tcd1x_h) in Write mode  --- \n\n", $time, test_num);
  for ( d_num = 0; d_num <= 3; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
     write_mem("",8'd0, 32'd0);
  ...
  $display ("\n#######################################################################");
  test_num = 175;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Constraint on D[7 : 4](Tcd2x_h) in Write mode  --- \n\n", $time, test_num);
  for ( d_num = 4; d_num <= 7; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
     write_mem("",8'd0, 32'd0);
  ...
  $display ("\n#######################################################################");
  test_num = 176;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Constraint on D[11 : 8](Tcd3x_h) in Write mode  --- \n\n", $time, test_num);
  for ( d_num = 8; d_num <= 11; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
     write_mem("",8'd0, 32'd0);
  ...
  $display ("\n#######################################################################");
  test_num = 177;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Constraint on D[15 : 12](Tcd4x_h) in Write mode  --- \n\n", $time, test_num);
  for ( d_num = 12; d_num <= 15; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
     write_mem("",8'd0, 32'd0);
  ...
  $display ("\n#######################################################################");
  test_num = 178;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Constraint on D[19 : 16](Tcd5x_h) in Write mode  --- \n\n", $time, test_num);
  for ( d_num = 16; d_num <= 19; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
     write_mem("",8'd0, 32'd0);
  ...
  $display ("\n#######################################################################");
  test_num = 179;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Constraint on D[23 : 20](Tcd6x_h) in Write mode  --- \n\n", $time, test_num);
  for ( d_num = 20; d_num <= 23; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
     write_mem("",8'd0, 32'd0);
  ...
  $display ("\n#######################################################################");
  test_num = 180;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Constraint on D[27 : 24](Tcd7x_h) in Write mode  --- \n\n", $time, test_num);
  for ( d_num = 24; d_num <= 27; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
     write_mem("",8'd0, 32'd0);
  ...
  $display ("\n#######################################################################");
  test_num = 181;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Constraint on D[31 : 28](Tcd8x_h) in Write mode  --- \n\n", $time, test_num);
  for ( d_num = 28; d_num <= 31; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
     write_mem("",8'd0, 32'd0);
  ...
  $display ("\n#######################################################################");
  test_num = 182;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[3 : 0](Tcd1x_h) in Write mode  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'd0);
  for ( d_num = 0; d_num <= 3; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 183;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[7 : 4](Tcd2x_h) in Write mode  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'd0);
  for ( d_num = 4; d_num <= 7; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 184;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[11 : 8](Tcd3x_h) in Write mode  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'd0);
  for ( d_num = 8; d_num <= 11; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 185;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[15 : 12](Tcd4x_h) in Write mode  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'd0);
  for ( d_num = 12; d_num <= 15; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 186;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[19 : 16](Tcd5x_h) in Write mode  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'd0);
  for ( d_num = 16; d_num <= 19; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 187;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[23 : 20](Tcd6x_h) in Write mode  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'd0);
  for ( d_num = 20; d_num <= 23; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 188;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[27 : 24](Tcd7x_h) in Write mode  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'd0);
  for ( d_num = 24; d_num <= 27; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 189;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[31 : 28](Tcd8x_h) in Write mode  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'd0);
  for ( d_num = 28; d_num <= 31; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 190;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[3 : 0](Td1c_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 0; d_num <= 3; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
    $display ("\n@%t  Check Setup Violation with D[%0d] bit", $time, d_num);
  ...
  $display ("\n#######################################################################");
  test_num = 191;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[7 : 4](Td2c_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 4; d_num <= 7; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 192;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[11 : 8](Td3c_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 8; d_num <= 11; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 193;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[15 : 12](Td4c_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 12; d_num <= 15; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 194;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[19 : 16](Td5c_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 16; d_num <= 19; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 195;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[23 : 20](Td6c_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 20; d_num <= 23; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 196;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[27 : 24](Td7c_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 24; d_num <= 27; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 197;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Setup Violation on D[31 : 28](Td8c_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 28; d_num <= 31; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 198;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[3 : 0](Tcd1x_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 0; d_num <= 3; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 199;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[7 : 4](Tcd2x_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 4; d_num <= 7; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 200;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[11 : 8](Tcd3x_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 8; d_num <= 11; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 201;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[15 : 12](Tcd4x_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 12; d_num <= 15; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 202;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[19 : 16](Tcd5x_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 16; d_num <= 19; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 203;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[23 : 20](Tcd6x_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 20; d_num <= 23; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 204;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[27 : 24](Tcd7x_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 24; d_num <= 27; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  $display ("\n#######################################################################");
  test_num = 205;
  $display( "\n\n@%t TEST: %d --- Going to check Bitwise Hold Violation on D[31 : 28](Tcd8x_h) in Write mode with output bits high  --- \n\n", $time, test_num);
  write_mem("",8'd0, 32'b11111111111111111111111111111111);
  for ( d_num = 28; d_num <= 31; d_num = d_num + 1 ) 
  begin
    @(negedge CLK);
  ...
  setuphold("WE","Tcwx",$realtobits(0.083),1,0,0,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 212;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WE  --- \n\n", $time, test_num);
  setuphold("WE","Tcwx",$realtobits(0.083),1,1,0,2'b10,2'b01,2'b10);
  @(negedge CLK);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 213;
  ...
  setuphold("TEST1","TT1C",$realtobits(0.047),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 218;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on TEST1 in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TT1C",$realtobits(0.047),0,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 219;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on TEST1 in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(1.379),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 220;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST1  in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(1.379),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 221;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on TEST1 in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TT1C",$realtobits(0.047),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 222;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on TEST1 in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TT1C",$realtobits(0.047),0,1,1,2'b01,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 223;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on TEST1 in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(1.379),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 224;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST1  in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(1.379),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 225;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on TEST_RNM in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Ttrnmc",$realtobits(0.047),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 226;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on TEST_RNM in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Ttrnmc",$realtobits(0.047),0,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 227;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on TEST_RNM in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(1.379),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 228;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST_RNM  in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(1.379),1,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 229;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on TEST_RNM in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Ttrnmc",$realtobits(0.047),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 230;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on TEST_RNM in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Ttrnmc",$realtobits(0.047),0,1,1,2'b01,2'b00,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 231;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on TEST_RNM in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(1.379),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 232;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST_RNM  in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(1.379),1,1,1,2'b10,2'b00,2'b11);
  @(negedge CLK);
  ...
  setuphold("RM","Trmc",$realtobits(0.177),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 234;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[2:0] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Trmc",$realtobits(0.177),0,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 235;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[2:0] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrmx",$realtobits(-0.009),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 236;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[2:0] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrmx",$realtobits(-0.009),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 237;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on RM[3] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.677),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 238;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[3] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.677),0,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 239;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[3] Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(1.379),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 240;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[3] Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(1.379),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  RM = 4'b0011;
  ...
  setuphold("RM","Trmc",$realtobits(0.177),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 242;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[2:0] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Trmc",$realtobits(0.177),0,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 243;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[2:0] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrmx",$realtobits(-0.009),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 244;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[2:0] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrmx",$realtobits(-0.009),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 245;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.677),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 246;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.677),0,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 247;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(1.379),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 248;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(1.379),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  RM = 4'b0011;
  @(negedge CLK);
  $display ("\n#######################################################################");
  ...
  setuphold("RME","Trmec",$realtobits(0.177),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
    RME = 1'b1;
  $display ("\n#######################################################################");
  test_num = 250;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RME in Read mode  --- \n\n", $time, test_num);
  setuphold("RME","Trmec",$realtobits(0.177),0,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 251;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RME in Read mode  --- \n\n", $time, test_num);
  setuphold("RME","Tcrmex",$realtobits(-0.009),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
    RME = 1'b1;
  $display ("\n#######################################################################");
  test_num = 252;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RME in Read mode  --- \n\n", $time, test_num);
  setuphold("RME","Tcrmex",$realtobits(-0.009),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 253;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on RME in Write mode  --- \n\n", $time, test_num);
  setuphold("RME","Trmec",$realtobits(0.177),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
    RME = 1'b1;
  $display ("\n#######################################################################");
  test_num = 254;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RME in Write mode  --- \n\n", $time, test_num);
  setuphold("RME","Trmec",$realtobits(0.177),0,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 255;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RME in Write mode  --- \n\n", $time, test_num);
  setuphold("RME","Tcrmex",$realtobits(-0.009),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
    RME = 1'b1;
  $display ("\n#######################################################################");
  test_num = 256;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RME in Write mode  --- \n\n", $time, test_num);
  setuphold("RME","Tcrmex",$realtobits(-0.009),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  RM = 4'b0011;
  RM = 4'b0011;
  ...
  setuphold("WA","Tcwax",$realtobits(1.379),1,0,0,2'b10,2'b00,2'b00);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 258;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WA in Read Mode  --- \n\n", $time, test_num);
  setuphold("WA","Tcwax",$realtobits(1.379),1,1,0,2'b10,2'b00,2'b00);
   read_mem("",8'd0,32'b0);
  ...
  setuphold("WA","Tcwax",$realtobits(1.379),1,0,1,2'b10,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 260;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WA in Write Mode  --- \n\n", $time, test_num);
  setuphold("WA","Tcwax",$realtobits(1.379),1,1,1,2'b10,2'b00,2'b11);
   read_mem("",8'd0,32'b0);
  ...
  setuphold("WPULSE","Tcwpx",$realtobits(1.379),1,0,1,2'b10,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 262;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WPULSE in Write Mode  --- \n\n", $time, test_num);
  setuphold("WPULSE","Tcwpx",$realtobits(1.379),1,1,1,2'b10,2'b00,2'b11);
   read_mem("",8'd0,32'b0);
  ...
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on BC0 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC0","Tbc0c",$realtobits(0.677),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 264;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on BC0 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC0","Tbc0c",$realtobits(0.677),0,1,0,2'b01,2'b00,2'b11);
   read_mem("",8'd0,32'b0);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 265;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on BC1 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC1","Tbc1c",$realtobits(0.263),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 266;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on BC1 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC1","Tbc1c",$realtobits(0.263),0,1,0,2'b01,2'b00,2'b11);
   read_mem("",8'd0,32'b0);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 267;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on BC2 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC2","Tbc2c",$realtobits(0.263),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 268;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on BC2 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC2","Tbc2c",$realtobits(0.263),0,1,0,2'b01,2'b00,2'b11);
   read_mem("",8'd0,32'b0);
  write_mem(1,"","");
  read_mem(1,"","");
`ifdef code_coverage
