...
//  Date of Generation : Thu Nov 10 09:50:36 CET 2022                  
//                                                                     
//---------------------------------------------------------------------
//   --------------------------------------------------------------     
...
//                                                                    
//   Memory Name:SNPS_RF_SP_UHS_256x28                                
//   Memory Size:256 words x 28 bits                                  
//                                                                    
//                               PORT NAME                            
//                               ---------                            
//               Output Ports                                         
...
SNPS_RF_SP_UHS_256x28_behav  top_behav (  .Q( Q_BEHAV), .ADR( ADR), .D( D), .WE( WE), .ME( ME), .CLK( CLK), .TEST1( TEST1), .TEST_RNM( TEST_RNM), .RME( RME), .RM( RM), .WA( WA), .WPULSE( WPULSE), .LS( LS), .BC0( BC0), .BC1( BC1), .BC2( BC2) );
SNPS_RF_SP_UHS_256x28  top_struct (  .Q( Q), .ADR( ADR), .D( D), .WE( WE), .ME( ME), .CLK( CLK), .TEST1( TEST1), .TEST_RNM( TEST_RNM), .RME( RME), .RM( RM), .WA( WA), .WPULSE( WPULSE), .LS( LS), .BC0( BC0), .BC1( BC1), .BC2( BC2) );
`define True    1'b1
`define False   1'b0
`define Mid     1'bx
`define Highz   1'bz
initial begin
  ...
  //$vcdpluson; 
    ...
    //$sdf_annotate( "SNPS_RF_SP_UHS_256x28.sdf");
end
task assist_initial;
input [8*8:1] signal;
begin
...
begin
  ...
  begin
    ...
    if (signal == "TEST1" && oper1 == 0 )
      TEST1 <= #1.1269999999999998 1'b1;
    else if ( signal == "TEST1" )
      TEST1 <= #1.1269999999999998 1'b0;
  if (signal == "TEST_RNM")
    TEST_RNM = 1'b1;
  if (signal == "RM" )
  begin
 ...
 if (signal == "ME_X")
  ...
  begin
    ...
    begin
      if ( chk == 2'b11 )
      begin
        read_mem("",8'd255,28'bx);
        write_mem("",8'd255,28'b0);
      end
      else
      begin
        read_mem("",8'd0,28'bx);
        write_mem("",8'd0,28'b0);
      end
    end
...
begin
  ...
  begin
    if ( memstat == 2'b00 )
      read_mem("",8'd0,28'b0000000000000000000000000000);
    else if ( memstat == 2'b01 )
      read_mem(1,"","");
    else if ( memstat == 2'b10 )
    begin
...
task coverage_1;
 ...
 @(posedge CLK) begin
     ...
     D[27:0] = pattern_reg[35:8];
     WE = pattern_reg[36];
     ME = pattern_reg[37];
     TEST1 = pattern_reg[38];
     TEST_RNM = pattern_reg[39];
     RME = pattern_reg[40];
     RM[3:0] = pattern_reg[44:41];
     WA[1:0] = pattern_reg[46:45];
     WPULSE[2:0] = pattern_reg[49:47];
     LS = pattern_reg[50];
     BC0 = pattern_reg[51];
     BC1 = pattern_reg[52];
     BC2 = pattern_reg[53];
...
begin
 ...
 $display("@%t  ============= When BC0 = %d ================ ", $time, bc0_i);
  ...
  write_mem("",8'b0,28'd0);
  @(negedge CLK);
  ADR = 1;
  $display("@%t    Writing 28'b1111111111111111111111111111 at %b", $time,ADR);
  write_mem("",8'b1,28'b1111111111111111111111111111);
...
WPULSE = 3'b000;
  ...
  read_mem("",8'b1, 28'b1111111111111111111111111111);
...
WPULSE = 3'b000;
 ...
 $display("Non recommended warning messages may come in simulation log file.");
  ...
  $display("@%t    Writing in Memory data = 28'b1010101010101010101010101010 during TEST1 active", $time);
   write_mem(1,8'bx,28'b1010101010101010101010101010); 
  @(negedge CLK);
  @(negedge CLK);
   read_mem_test1(1,8'bx,28'b1010101010101010101010101010); 
  if (err_cnt == 0)
  begin
...
WPULSE = 3'b000;
  ...
  setuphold("D","Tdc",$realtobits(0.038),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 155;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on D in Read mode  --- \n\n", $time, test_num);
  setuphold("D","Tdc",$realtobits(0.038),0,1,0,2'b00,2'b00,2'b00);
  $display ("\n#######################################################################");
  test_num = 156;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on D in Write mode  When Write Enable Inactive --- \n\n", $time, test_num);
  setuphold("D_WE0","Tdc",$realtobits(0.038),0,1,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 157;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on D in Read mode  --- \n\n", $time, test_num);
  setuphold("D","Tcdx",$realtobits(0.069),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 158;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on D in Read mode  --- \n\n", $time, test_num);
  setuphold("D","Tcdx",$realtobits(0.069),1,1,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 159;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on D in Write mode  When Write Enable Inactive --- \n\n", $time, test_num);
  setuphold("D_WE0","Tcdx",$realtobits(0.069),1,1,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 160;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on D in Write mode  --- \n\n", $time, test_num);
  setuphold("D","Tdc",$realtobits(0.038),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 161;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on D in Write mode  --- \n\n", $time, test_num);
  setuphold("D","Tdc",$realtobits(0.038),0,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 162;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on D in Write mode  --- \n\n", $time, test_num);
  setuphold("D","Tcdx",$realtobits(0.069),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 163;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on D in Write mode  --- \n\n", $time, test_num);
  setuphold("D","Tcdx",$realtobits(0.069),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 164;
  ...
  write_mem("",8'd0, 28'b1111111111111111111111111111);
  for ( d_num = 0; d_num < 28; d_num = d_num + 1 )
  begin
    @(negedge CLK);
    $display ("\n@%t  Check Setup Violation with D[%0d] bit", $time, d_num);
  ...
  setuphold("WE","Twc",$realtobits(0.033),0,0,0,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 171;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on WE  --- \n\n", $time, test_num);
  setuphold("WE","Twc",$realtobits(0.033),0,1,0,2'b10,2'b01,2'b10);
  @(negedge CLK);
  @(negedge CLK);
  @(negedge CLK);
  $display ("\n#######################################################################");
  ...
  setuphold("WE_X","Twc",$realtobits(0.033),0,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  @(negedge CLK);
  write_mem(1,"","");
  ...
  setuphold("TEST1","TCT1X",$realtobits(1.126),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 182;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST1  in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(1.126),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 183;
  ...
  setuphold("TEST1","TCT1X",$realtobits(1.126),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 186;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST1  in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(1.126),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 187;
  ...
  setuphold("TEST_RNM","Tctrnmx",$realtobits(1.126),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 190;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST_RNM  in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(1.126),1,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 191;
  ...
  setuphold("TEST_RNM","Tctrnmx",$realtobits(1.126),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 194;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST_RNM  in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(1.126),1,1,1,2'b10,2'b00,2'b11);
  @(negedge CLK);
  ...
  setuphold("RM","Trmc",$realtobits(0.150),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 196;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[2:0] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Trmc",$realtobits(0.150),0,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 197;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[2:0] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrmx",$realtobits(0.025),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 198;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[2:0] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrmx",$realtobits(0.025),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 199;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on RM[3] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.293),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 200;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[3] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.293),0,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 201;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[3] Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(1.126),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 202;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[3] Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(1.126),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  RM = 4'b0011;
  ...
  setuphold("RM","Trmc",$realtobits(0.150),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 204;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[2:0] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Trmc",$realtobits(0.150),0,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 205;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[2:0] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrmx",$realtobits(0.025),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 206;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[2:0] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrmx",$realtobits(0.025),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 207;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.293),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 208;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.293),0,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 209;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(1.126),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 210;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(1.126),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  RM = 4'b0011;
  @(negedge CLK);
  $display ("\n#######################################################################");
  ...
  setuphold("RME","Trmec",$realtobits(0.150),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
    RME = 1'b1;
  $display ("\n#######################################################################");
  test_num = 212;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RME in Read mode  --- \n\n", $time, test_num);
  setuphold("RME","Trmec",$realtobits(0.150),0,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 213;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RME in Read mode  --- \n\n", $time, test_num);
  setuphold("RME","Tcrmex",$realtobits(0.025),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
    RME = 1'b1;
  $display ("\n#######################################################################");
  test_num = 214;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RME in Read mode  --- \n\n", $time, test_num);
  setuphold("RME","Tcrmex",$realtobits(0.025),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 215;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on RME in Write mode  --- \n\n", $time, test_num);
  setuphold("RME","Trmec",$realtobits(0.150),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
    RME = 1'b1;
  $display ("\n#######################################################################");
  test_num = 216;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RME in Write mode  --- \n\n", $time, test_num);
  setuphold("RME","Trmec",$realtobits(0.150),0,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 217;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RME in Write mode  --- \n\n", $time, test_num);
  setuphold("RME","Tcrmex",$realtobits(0.025),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
    RME = 1'b1;
  $display ("\n#######################################################################");
  test_num = 218;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RME in Write mode  --- \n\n", $time, test_num);
  setuphold("RME","Tcrmex",$realtobits(0.025),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  RM = 4'b0011;
  RM = 4'b0011;
  ...
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on WA in Read Mode  --- \n\n", $time, test_num);
   setuphold("WA","Twac",$realtobits(0.150),0,0,0,2'b10,2'b00,2'b00);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 220;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on WA in Read Mode  --- \n\n", $time, test_num);
   setuphold("WA","Twac",$realtobits(0.150),0,1,0,2'b10,2'b00,2'b00);
   read_mem("",8'd0,28'b0);

  $display ("\n#######################################################################");
  test_num = 221;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on WA in Read mode  --- \n\n", $time, test_num);
  setuphold("WA","Tcwax",$realtobits(1.126),1,0,0,2'b10,2'b00,2'b00);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 222;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WA in Read Mode  --- \n\n", $time, test_num);
  setuphold("WA","Tcwax",$realtobits(1.126),1,1,0,2'b10,2'b00,2'b00);
   read_mem("",8'd0,28'b0);
  ...
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on WA in Write Mode  --- \n\n", $time, test_num);
   setuphold("WA","Twac",$realtobits(0.150),0,0,1,2'b10,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 224;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on WA in Write Mode  --- \n\n", $time, test_num);
   setuphold("WA","Twac",$realtobits(0.150),0,1,1,2'b10,2'b00,2'b11);
   read_mem("",8'd0,28'b0);

  $display ("\n#######################################################################");
  test_num = 225;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on WA in Write mode  --- \n\n", $time, test_num);
  setuphold("WA","Tcwax",$realtobits(1.126),1,0,1,2'b10,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 226;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WA in Write Mode  --- \n\n", $time, test_num);
  setuphold("WA","Tcwax",$realtobits(1.126),1,1,1,2'b10,2'b00,2'b11);
   read_mem("",8'd0,28'b0);
  ...
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on WPULSE in Write Mode  --- \n\n", $time, test_num);
   setuphold("WPULSE","Twpc",$realtobits(0.150),0,0,1,2'b10,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 228;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on WPULSE in Write Mode  --- \n\n", $time, test_num);
   setuphold("WPULSE","Twpc",$realtobits(0.150),0,1,1,2'b10,2'b00,2'b11);
   read_mem("",8'd0,28'b0);

  $display ("\n#######################################################################");
  test_num = 229;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on WPULSE in Write mode  --- \n\n", $time, test_num);
  setuphold("WPULSE","Tcwpx",$realtobits(1.126),1,0,1,2'b10,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 230;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WPULSE in Write Mode  --- \n\n", $time, test_num);
  setuphold("WPULSE","Tcwpx",$realtobits(1.126),1,1,1,2'b10,2'b00,2'b11);
   read_mem("",8'd0,28'b0);
  ...
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on BC0 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC0","Tbc0c",$realtobits(0.293),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 232;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on BC0 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC0","Tbc0c",$realtobits(0.293),0,1,0,2'b01,2'b00,2'b11);
   read_mem("",8'd0,28'b0);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 233;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on BC1 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC1","Tbc1c",$realtobits(0.154),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 234;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on BC1 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC1","Tbc1c",$realtobits(0.154),0,1,0,2'b01,2'b00,2'b11);
   read_mem("",8'd0,28'b0);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 235;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on BC2 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC2","Tbc2c",$realtobits(0.154),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 236;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on BC2 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC2","Tbc2c",$realtobits(0.154),0,1,0,2'b01,2'b00,2'b11);
   read_mem("",8'd0,28'b0);
  write_mem(1,"","");
  read_mem(1,"","");
`ifdef code_coverage
