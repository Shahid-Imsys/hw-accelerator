...
//  Date of Generation : Wed May 04 16:41:10 CEST 2022                 
//                                                                     
//---------------------------------------------------------------------
//   --------------------------------------------------------------     
...
SNPS_SP_HD_512x8_behav  top_behav (  .Q( Q_BEHAV), .ADR( ADR), .D( D), .WE( WE), .ME( ME), .CLK( CLK), .TEST1( TEST1), .TEST_RNM( TEST_RNM), .RME( RME), .RM( RM), .WA( WA), .WPULSE( WPULSE), .LS( LS), .BC0( BC0), .BC1( BC1), .BC2( BC2) );
SNPS_SP_HD_512x8  top_struct (  .Q( Q), .ADR( ADR), .D( D), .WE( WE), .ME( ME), .CLK( CLK), .TEST1( TEST1), .TEST_RNM( TEST_RNM), .RME( RME), .RM( RM), .WA( WA), .WPULSE( WPULSE), .LS( LS), .BC0( BC0), .BC1( BC1), .BC2( BC2) );
`define True    1'b1
`define False   1'b0
`define Mid     1'bx
`define Highz   1'bz
initial begin
  ...
  //$vcdpluson; 
    ...
    //$sdf_annotate( "SNPS_SP_HD_512x8.sdf");
end
task print_tdcs;
input count;
...
task write_mem;
input [8:0] addr_incr; //address increments by this value
input [8:0] address_w; //if null,entire memory is written.
...
task read_mem;
input [8:0] addr_incr; //address increments by this value
input [8:0] address_r; //if null,entire memory is read.
input [7:0] data; //if null,data=0 in case of single location,else data=addr.
integer k;
integer j;
integer l;
...
task read_mem_inv;
input [8:0] addr_incr; //address increments by this value
input [8:0] addr; //if null,entire memory is read.
input [7:0] data; //if null,data=0 in case of single location,else data=addr.
integer k;
integer j;
reg [7:0]  i;
 ...
 if (signal == "ME_X")
  ...
  begin
    ...
    begin
      if ( chk == 2'b11 )
      begin
        read_mem("",9'd511,8'bx);
        write_mem("",9'd511,8'b0);
      end
      else
      begin
        read_mem("",9'd0,8'bx);
...
task coverage_1;
 ...
 @(posedge CLK) begin
     ADR[8:0] = pattern_reg[8:0];
     D[7:0] = pattern_reg[16:9];
     WE = pattern_reg[17];
     ME = pattern_reg[18];
     TEST1 = pattern_reg[19];
     TEST_RNM = pattern_reg[20];
     RME = pattern_reg[21];
     RM[3:0] = pattern_reg[25:22];
     WA[1:0] = pattern_reg[27:26];
     WPULSE[2:0] = pattern_reg[30:28];
     LS = pattern_reg[31];
     BC0 = pattern_reg[32];
     BC1 = pattern_reg[33];
     BC2 = pattern_reg[34];
...
task coverage_2;
  ...
  @(negedge CLK) begin
     #5;
     ADR[8:0] = add_data_pattern[8:0];
     D[7:0] = add_data_pattern[16:9];
     WE = control_sig[0];
     ME = control_sig[1];
     TEST1 = control_sig[2];
     LS <= control_sig[3];
...
WPULSE = 3'b000;
 ...
 $display("Non recommended warning messages may come in simulation log file.");
  ...
  $display("@%t    Writing in Memory data = 8'b10101010 during TEST1 active", $time);
   write_mem(1,9'bx,8'b10101010); 
  @(negedge CLK);
  @(negedge CLK);
   read_mem_test1(1,9'bx,8'b10101010); 
  if (err_cnt == 0)
  begin
...
WPULSE = 3'b000;
  ...
  setuphold("ADR","Tcax",$realtobits(0.060),1,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 113;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on ADR in Read mode  --- \n\n", $time, test_num);
  setuphold("ADR","Tcax",$realtobits(0.060),1,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 114;
  ...
  setuphold("ADR","Tcax",$realtobits(0.060),1,0,1,2'b01,2'b00,2'b01);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 117;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on ADR in Write mode  --- \n\n", $time, test_num);
  setuphold("ADR","Tcax",$realtobits(0.060),1,1,1,2'b10,2'b00,2'b11);
  @(negedge CLK);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 118;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on LS when BC0 = 0 in Write mode  --- \n\n", $time, test_num);
  setuphold("LS","Tfls0c",$realtobits(0.407),0,0,1,2'b01,2'b00,2'b01);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 119;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on LS when BC0 = 0 in Write mode  --- \n\n", $time, test_num);
  setuphold("LS","Tfls0c",$realtobits(0.407),0,1,1,2'b10,2'b01,2'b11);
  $display ("\n#######################################################################");
  test_num = 120;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on LS when BC0 = 0 in Read mode  --- \n\n", $time, test_num);
  setuphold("LS","Tfls0c",$realtobits(0.407),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 121;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on LS when BC0 = 0 in Read mode  --- \n\n", $time, test_num);
  setuphold("LS","Tfls0c",$realtobits(0.407),0,1,0,2'b10,2'b01,2'b11);
  ...
  setuphold("LS","Tlsx",$realtobits(0.066),1,0,1,2'b01,2'b00,2'b01);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 127;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on LS when BC0 = 0 in Write mode  --- \n\n", $time, test_num);
  setuphold("LS","Tlsx",$realtobits(0.066),1,1,1,2'b10,2'b01,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 128;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on LS when BC0 = 0 in Read mode  --- \n\n", $time, test_num);
  setuphold("LS","Tlsx",$realtobits(0.066),1,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 129;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on LS when BC0 = 0 in Read mode  --- \n\n", $time, test_num);
  setuphold("LS","Tlsx",$realtobits(0.066),1,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 130;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on LS when BC0 = 1 in Write mode  --- \n\n", $time, test_num);
  setuphold("LS_BC0_1","Tlsx",$realtobits(0.066),1,0,1,2'b01,2'b00,2'b01);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 131;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on LS when BC0 = 1 in Write mode  --- \n\n", $time, test_num);
  setuphold("LS_BC0_1","Tlsx",$realtobits(0.066),1,1,1,2'b10,2'b01,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 132;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on LS when BC0 = 1 in Read mode  --- \n\n", $time, test_num);
  setuphold("LS_BC0_1","Tlsx",$realtobits(0.066),1,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 133;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on LS when BC0 = 1 in Read mode  --- \n\n", $time, test_num);
  setuphold("LS_BC0_1","Tlsx",$realtobits(0.066),1,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  ...
  setuphold("WE","Twc",$realtobits(0.052),0,0,0,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 209;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on WE  --- \n\n", $time, test_num);
  setuphold("WE","Twc",$realtobits(0.052),0,1,0,2'b10,2'b01,2'b10);
  @(negedge CLK);
  @(negedge CLK);
  @(negedge CLK);
  $display ("\n#######################################################################");
  ...
  setuphold("WE_X","Twc",$realtobits(0.052),0,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  @(negedge CLK);
  write_mem(1,"","");
  ...
  setuphold("WE","Tcwx",$realtobits(0.059),1,0,0,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 212;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WE  --- \n\n", $time, test_num);
  setuphold("WE","Tcwx",$realtobits(0.059),1,1,0,2'b10,2'b01,2'b10);
  @(negedge CLK);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 213;
  ...
  setuphold("TEST1","TT1C",$realtobits(0.031),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 218;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on TEST1 in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TT1C",$realtobits(0.031),0,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 219;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on TEST1 in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(0.896),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 220;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST1  in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(0.896),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 221;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on TEST1 in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TT1C",$realtobits(0.031),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 222;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on TEST1 in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TT1C",$realtobits(0.031),0,1,1,2'b01,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 223;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on TEST1 in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(0.896),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 224;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST1  in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST1","TCT1X",$realtobits(0.896),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 225;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on TEST_RNM in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Ttrnmc",$realtobits(0.031),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 226;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on TEST_RNM in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Ttrnmc",$realtobits(0.031),0,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 227;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on TEST_RNM in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(0.896),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 228;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST_RNM  in Read mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(0.896),1,1,0,2'b10,2'b01,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 229;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on TEST_RNM in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Ttrnmc",$realtobits(0.031),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 230;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on TEST_RNM in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Ttrnmc",$realtobits(0.031),0,1,1,2'b01,2'b00,2'b11);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 231;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on TEST_RNM in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(0.896),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 232;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on TEST_RNM  in Write mode  --- \n\n", $time, test_num);
  setuphold("TEST_RNM","Tctrnmx",$realtobits(0.896),1,1,1,2'b10,2'b00,2'b11);
  @(negedge CLK);
  ...
  setuphold("RM","Trm3c",$realtobits(0.407),0,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 238;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[3] in Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.407),0,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 239;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[3] Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(0.896),1,0,0,2'b00,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 240;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[3] Read mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(0.896),1,1,0,2'b10,2'b01,2'b00);
  @(negedge CLK);
  RM = 4'b0011;
  ...
  setuphold("RM","Trm3c",$realtobits(0.407),0,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 246;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Trm3c",$realtobits(0.407),0,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 247;
  $display( "\n\n@%t TEST: %d --- Going to check Hold constraint on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(0.896),1,0,1,2'b01,2'b00,2'b00);
  @(negedge CLK);
  $display ("\n#######################################################################");
  test_num = 248;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on RM[3] in Write mode  --- \n\n", $time, test_num);
  setuphold("RM","Tcrm3x",$realtobits(0.896),1,1,1,2'b10,2'b00,2'b10);
  @(negedge CLK);
  RM = 4'b0011;
  @(negedge CLK);
  $display ("\n#######################################################################");
  ...
  setuphold("WA","Tcwax",$realtobits(0.896),1,0,0,2'b10,2'b00,2'b00);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 258;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WA in Read Mode  --- \n\n", $time, test_num);
  setuphold("WA","Tcwax",$realtobits(0.896),1,1,0,2'b10,2'b00,2'b00);
   read_mem("",9'd0,8'b0);
  ...
  setuphold("WA","Tcwax",$realtobits(0.896),1,0,1,2'b10,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 260;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WA in Write Mode  --- \n\n", $time, test_num);
  setuphold("WA","Tcwax",$realtobits(0.896),1,1,1,2'b10,2'b00,2'b11);
   read_mem("",9'd0,8'b0);
  ...
  setuphold("WPULSE","Tcwpx",$realtobits(0.896),1,0,1,2'b10,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 262;
  $display( "\n\n@%t TEST: %d --- Going to check Hold Violation on WPULSE in Write Mode  --- \n\n", $time, test_num);
  setuphold("WPULSE","Tcwpx",$realtobits(0.896),1,1,1,2'b10,2'b00,2'b11);
   read_mem("",9'd0,8'b0);
  ...
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on BC0 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC0","Tbc0c",$realtobits(0.407),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 264;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on BC0 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC0","Tbc0c",$realtobits(0.407),0,1,0,2'b01,2'b00,2'b11);
   read_mem("",9'd0,8'b0);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 265;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on BC1 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC1","Tbc1c",$realtobits(0.097),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 266;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on BC1 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC1","Tbc1c",$realtobits(0.097),0,1,0,2'b01,2'b00,2'b11);
   read_mem("",9'd0,8'b0);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 267;
  $display( "\n\n@%t TEST: %d --- Going to check Setup constraint on BC2 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC2","Tbc2c",$realtobits(0.097),0,0,0,2'b01,2'b00,2'b01);
  @(negedge CLK);
   $display ("\n#######################################################################");
  test_num = 268;
  $display( "\n\n@%t TEST: %d --- Going to check Setup Violation on BC2 in Light Sleep Mode  --- \n\n", $time, test_num);
   setuphold("BC2","Tbc2c",$realtobits(0.097),0,1,0,2'b01,2'b00,2'b11);
   read_mem("",9'd0,8'b0);
  write_mem(1,"","");
  read_mem(1,"","");
`ifdef code_coverage
